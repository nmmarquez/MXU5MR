...
## Background  
In this debrief we unpack some of the interesting results from examining the change in under 5 mortality in the forthcoming paper, Under 5 Mortality Estimates For Mexico’s Municipalities: 2000-2015. This debrief assesses the variation of child health outcomes experienced in the years 2000 to 2015 by examining$~_5q_0$, the probability of death before age 5, at the municipality level within Mexico. In 2016, at the end of the tracking period for the Millennium Development Goals (MDGs), the World Health Organization released a report stating that, of the 75 priority countries followed by an ongoing analysis of child mortality monitoring, Mexico was one of twenty five countries that successfully achieved the goal of reducing under 5 mortality by two thirds. During the period between 1990 and 2015 Mexico reduced their national under 5 mortality rate from 48 to 16 deaths per 1000 live births. Mexico's 2456 municipalities contain diverse populations and by examining how municipalities contributed to the change in national level child mortality, we can gain a better understanding of how subpopulations differentially experience child mortality events and how this has changed over time. This is especially important in the context of the Sustainable Development Goals where the focus has been shifted to the reduction of national levels of inequality of various social and economic measures, rather than absolute reduction at the national level. 
...
Q^a \sim \text{AR}_1(\rho^a) \\
Q^t \sim \text{AR}_1(\rho^t) \\
Q^l \sim \text{LCAR}(\rho^l, \sigma)
$$

Where $\phi$ is a set of random effects that describe the latent age-spatiotemporal correlations that exist in the underlying mortality rates. These random effects have the effect of smoothing data such that outliers and improbably low or high observed rates of age specific mortality in a given year for a particular geography due to small sample size are adjusted base on the correlations observed in the dataset as a whole. The model produces a set of parameters $\lambda_{l, a, t}$ which are age specific mortality rates, for specific municipality-year which then can be converted to$~_5q_0$ using traditional demographic methods. The benefit to this approach is that it allows us to take into account the correlated structure of the mortality events that we observe as well as leverage the uncertainty in our estimates that are attributable to variable sample sizes across different municipalities and years.

In order to remain consistent with current demographic estimates of mortality, we raked our results such that they matched the Global Burden of Disease(GBD) 2016 state level estimates of child mortality in Mexico, though, sensibility checks of aggregated data had estimates produced in this analysis closeley mathcing the GBD results.
...
pacman::p_load(INLA, TMB, raster, data.table, ggplot2, dplyr, dtplyr, ineq,
               ...
               stringr, leaflet, latex2exp, reldist)
...
load("~/Documents/MXU5MR/analysis/outputs/regDelaySPDF.Rdata")
...
Before municipality level results can be examined we address the variation in the data quality that exists from the Mexico Vital Registration Data from INEGI. For the years between 1995 and 2015 information on the delay of registration from time of birth was collected for each birth record and averaged across the time series for each municipality. Average registration time for municipalities ranges from less than one month to over three years. These results show significant geospataial autocorrelation, as measured by a Morans I test, with significant clusters of delayed reporting municipalities present in Chihuahua, Guerrero, and Chiapas. These delays would could result in biases in both the denominator and the numerator when trying to estimate annual age specific mortality, though, the potential effects are difficult to quantify.


```{R regDelay, echo=FALSE, warning=FALSE, message=FALSE}
col <- "REG_DIFFN"
regDelaySPDF@data$GEOID <- as.numeric(regDelaySPDF@data$GEOID)
regDelaySPDF@data$data <- regDelaySPDF@data[, col]
regDelaySPDF@data <- left_join(regDelaySPDF@data, DFName)

popupregDelay <- paste0(
    "State: ", regDelaySPDF@data$State,
    "<br>Municipality: ", regDelaySPDF@data$NOM_MUN,
    "<br>Registration Delay: ", round(regDelaySPDF@data$data, 4))

palrd <- colorNumeric(palette = "Spectral", domain = regDelaySPDF@data$data,
                    reverse=TRUE)
mapRegDelay <- leaflet() %>% addProviderTiles("CartoDB.Positron") %>%
    addPolygons(data=regDelaySPDF, fillColor=~palrd(data), color = "#b2aeae",
        weight = 0.3, fillOpacity = 0.7, smoothFactor = 0.2,
        popup = popupregDelay) %>%
    addLegend("bottomleft", pal = palrd,
    values = regDelaySPDF$data,
    title ="Registration <br>Time (Years)", opacity = 1)
...
pineqR <- ggplot(DTineq, aes(x=year, y=relineq)) + geom_line() +
    ...
    labs(x="Year", y="Relative Inequality") +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme_classic()
...
giniArr <- sapply(
    1:dim(q0array)[2], function(i) sapply(1:1000, function(j){
...
map1 <- leaflet() %>% addProviderTiles("CartoDB.Positron") %>%
    ...
    values = df$data, title ="Mexico 5q0<br>(2015)", opacity = 1)
...
It is interseting to note that an almost equal level of municipality variance is seen in Oaxaca comapred to the national level. In fact if we examine inequality metrics for just Oaxaca, we find that there has been no significant change in the levels of relative inequality, as measured by the Gini coefficient, which is shown in the time series plot below. Looking at state level measures of inequality is especially important in order to determine the potential variable impact that the state level ministries of health have on their population when state level health authorities exist such as in Mexico.

```{R oaxIneq, echo=FALSE, warning=FALSE, message=FALSE}
## Oaxaca Inequality
...
If you wish to view a full set of results, time series plots and maps for all years at national, state and municipality levels, they may be viewed at the following link.
