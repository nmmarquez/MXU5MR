...
statedf <- DT[,mean(Ratem1pop1), by=list(ENT_RESID, EDAD, YEAR)]
...
mx.sp.df@data <- left_join(mx.sp.df@data, subset(DT, EDAD==0 & YEAR==2015))
mx.sp.df@data$logRR <- log(mx.sp.df@data$Ratem1pop1)

spdf2leaf(mx.sp.df, "Ratem1pop1", "2015 U1MR<br>Relative<br>Risk")
spdf2leaf(mx.sp.df, "logRR", "2015 U1MR<br>Relative<br>Risk (log)")
